    ...
    return (
        <div className="flex h-screen bg-neutral-950 text-white overflow-hidden font-sans selection:bg-purple-500/30">
            ...
            <aside className="w-64 hidden md:flex flex-col border-r border-white/10 bg-neutral-900/50 backdrop-blur-xl relative z-10">
                <AdminSidebarContent user={user} handleLogout={handleLogout} />
            </aside>
            ...
            {sidebarOpen && (
                <div className="fixed inset-0 z-50 md:hidden flex">
                    ...
                    />
                    <aside className="w-64 bg-neutral-900 border-r border-white/10 relative z-10 flex flex-col h-full animate-slideRight">
                        <AdminSidebarContent user={user} handleLogout={handleLogout} />
                    </aside>
                </div>
            )}
            ...
            <main className="flex-1 flex flex-col relative overflow-hidden">
                ...
                {/* Mobile Header */}
                <header className="h-16 border-b border-white/5 bg-neutral-900/30 backdrop-blur-md flex items-center justify-between px-6 md:hidden z-20">
                    <div className="font-bold text-lg flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white text-xs font-bold">T</div>
                        Termiziy AI
                    </div>
                    <button
                        onClick={() => setSidebarOpen(!sidebarOpen)}
                        className="p-2 -mr-2 text-white/60 hover:text-white active:bg-white/5 rounded-lg transition-colors"
                    >
                        <Menu size={24} />
                    </button>
                </header>
...
function AdminSidebarContent({ user, handleLogout }: { user: User, handleLogout: () => void }) {
    return (
        <>
